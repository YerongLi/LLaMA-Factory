...
for i in tqdm(range(0, len(data), batch_size)):
    ...
    batch_response_inputs = tokenizer([item.get("response", '') for item in batch_data], return_tensors="pt", padding=True, truncation=True).to(device_str)
    # batch_response_inputs = tokenizer([item.get("gpt35_response", '') for item in batch_data], return_tensors="pt", padding=True, truncation=True).to(device_str)
