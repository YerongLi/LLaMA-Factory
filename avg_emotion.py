...
# Print the averages
print("Average output_positive_counts: {:.2f}".format(output_positive_average))
print("Average output_negative_counts: {:.2f}".format(output_negative_average))
print("Average response_positive_counts: {:.2f}".format(response_positive_average))
print("Average response_negative_counts: {:.2f}".format(response_negative_average))
...
with open('user4.jsonl', 'r') as jsonl_file:
    ...
    for line in tqdm.tqdm(jsonl_file):
        ...
        # Increment total lines
        total_lines += 1
        if not(len(json_obj['history']) > 2 and json_obj['history'][-2][0] == 'User'): continue
        # Get the positive and negative keyword counts for 'output' and 'response'
        output_positive_counts = len(json_obj['output_positive_keywords'])
        output_negative_counts = len(json_obj['output_negative_keywords'])
        response_positive_counts = len(json_obj['response_positive_keywords'])
